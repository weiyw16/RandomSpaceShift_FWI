...
int main(int argc, char* argv[]) {
  ...
  for(int is=rank*k; is<rank*k+ntask; is++) {
    ...
    std::vector<float> p1(exvel.nz * exvel.nx, 0);
		/*
    std::vector<float> pp0(exvel.nz * exvel.nx, 0);
    std::vector<float> pp1(exvel.nz * exvel.nx, 0);
		*/
    ...
    ShotPosition curSrcPos = allSrcPos.clipRange(is, is);
		...
		/*
		for(int ix = 0 ; ix < exvel.nx ; ix ++)
			for(int iz = 0 ; iz < exvel.nz ; iz ++) {
				p0[ix * exvel.nz + iz] = rand() % 1000 / 1.0;
				pp0[ix * exvel.nz + iz] = p0[ix * exvel.nz + iz];	
				p1[ix * exvel.nz + iz] = rand() % 1000 / 1.0;
				pp1[ix * exvel.nz + iz] = p1[ix * exvel.nz + iz];	
			}
			*/
    for(int it=0; it<nt; it++) {
      ...
      fmMethod.recordSeis(&dobs_trans[it*ng], &p0[0]);
			/*
      fmMethod.addSource(&pp1[0], &wlt[it], curSrcPos);
      fmMethod.swStepForward(pp0, pp1);
      std::swap(pp1, pp0);
			*/
    }
		...
		int count = 0;
		const int d = 6;
		for(int ix = d ; ix < exvel.nx - d ; ix ++) {
			for(int iz = d ; iz < exvel.nz - d ; iz ++) {
				if(count == 10)
					break;
				if(fabs(pp1[ix * exvel.nz + iz] - p1[ix * exvel.nz + iz]) > 1e-4) {
					printf("error: %d %d %f %f\n", ix, iz, pp1[ix * exvel.nz + iz], p1[ix * exvel.nz + iz]);
					count ++;
				}
			}
		}
