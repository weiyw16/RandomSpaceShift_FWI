...
Params::Params() {
  ...
  if (!sf_getint("flo", &flo))   { flo = -1; }                 /* low frequency in bandpass */
  if (!sf_getint("fhi", &fhi))   { fhi = -1; }                 /* high frequency in bandpass */
...
int main(int argc, char *argv[]) {
  ...
  /*
	sf_file sf_wlt = sf_output("wlt1.rsf");
	sf_putint(sf_wlt, "n1", nt);
	sf_putfloat(sf_wlt, "d1", dt);
  ...
  */
  ...
  /*
	if(flo != -1 && fhi != -1) 
		filter(&wlt[0], nt, dt, flo, fhi, phase, verb);
    */

  /*
	sf_wlt = sf_output("wlt2.rsf");
	sf_putint(sf_wlt, "n1", nt);
	sf_putfloat(sf_wlt, "d1", dt);
	sf_floatwrite(&wlt[0], nt, sf_wlt);
  */
  ...
  UpdateSteplenOp updateSteplenOp(fmMethod, updatevelop, nita, maxdv, fhi);
  ...
  float obj_desc = 0.0;
  FILE *f = fopen("iters.txt", "w");
  for (int iter = 0; iter < params.niter ; iter++) {
    essfwi.epoch(iter, 0, 0, fhi);
    ...
    /*
    if(iter == 9) {
      obj_desc = absobj[0] - absobj[9];
      if(obj_desc < 0) {
        INFO() << "=======================================================";
        INFO() << "The objective function is not descending, program stop";
        INFO() << "=======================================================";
        fprintf(f, "%d", iter);
        fclose(f);
        break;
    ...
    else if (iter % 5 == 0 && iter > 30) {
      if(absobj[iter - 9] - absobj[iter] < obj_desc * 0.04) {
        INFO() << "=======================================================";
        INFO() << "The objective function is not descending, program stop";
        INFO() << "=======================================================";
        fprintf(f, "%d", iter);
